...
- 使用相同網域、裝置、瀏覽器及瀏覽器使用者設定檔時，一般 App、快取及
  Service Worker 更新會保留 IndexedDB 中的收藏與照片。
- 一般版本更新不需要重新匯出或匯入 JSON；資料庫結構變更時，App 會透過已測試的
  migration 保留舊資料。
- JSON 是復原用安全備份，不是更新 App 的必要步驟。請定期匯出，並在清除網站資料、
  更換裝置／瀏覽器／production domain 或安裝重大版本前另外保存。
- 「保護本機資料」會向瀏覽器要求持久儲存，可降低自動清除的機率，但無法防止手動
  刪除、裝置損壞或遺失。
