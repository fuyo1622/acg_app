...
- The canonical production URL is `https://acg-app-steel.vercel.app/`.
...
- The app icons, including `public/apple-touch-icon.png`, are original geometric assets generated by `scripts/generate-icons.ps1`.
- The Vite config intentionally does not set `base` because Vercel serves the app from the domain root.
...
- Production routing, PWA assets, security headers, service-worker replacement, installation, offline restart, and local data persistence were verified during the 2026-07-23 public-release pass.

## Documentation map
- `docs/README.md`: Current documentation index.
- `docs/project-spec.md`: Canonical current project spec.
- `README.md` and `README.zh-TW.md`: English and Traditional Chinese project entry points with installation steps.
- `docs/install-guide.md`: User-facing Android/iOS installation steps and manual PWA verification checklist.
- `docs/architecture-decisions.md`: Current architecture decisions and tradeoffs.
- `docs/public-release-readiness.zh-TW.md`: Completed P0 release record and prioritized P1 roadmap.
